...
@dataclass
class StringItem:
    string_no:int;modules:int;estimated_vmp:float;estimated_voc:float;estimated_imp:float;mppt:Optional[int]=None;input_no:Optional[int]=None
    def to_dict(self):return asdict(self)
@dataclass
class StringDesign:
    total_modules:int;vmp:float;voc:float;imp:float;isc:float;target_voltage:float;max_modules_per_string:int;number_of_strings:int;strings:list[StringItem];num_mppt:int;inputs_per_mppt:int;warnings:list[str];checks:list[dict]
    @property
...
def _allocate_mppts(strings,num_mppt,inputs_per_mppt):
    warnings=[]
    if num_mppt<=0:
    ...
    capacity=num_mppt*inputs_per_mppt
    if len(strings)>capacity:warnings.append(f'Calculated {len(strings)} strings but the stored inverter input capacity is {capacity} ({num_mppt} MPPT × {inputs_per_mppt} input/MPPT). Overflow strings remain unassigned.')
    ...
    for s in sorted(strings,key=lambda s:(-s.modules,s.string_no)):
        ...
        idx=min(candidates,key=lambda i:(len(buckets[i]),sum(x.modules for x in buckets[i]),i));buckets[idx].append(s)
    for mppt_idx,bucket in enumerate(buckets,start=1):
        bucket.sort(key=lambda s:s.string_no)
        for inp_idx,s in enumerate(bucket,start=1):s.mppt=mppt_idx;s.input_no=inp_idx
        if len({s.modules for s in bucket})>1:warnings.append(f'MPPT{mppt_idx} contains strings with different module counts. Review the allocation if those strings will operate in parallel on the same tracker.')
    return warnings

def calculate_string_design(total_modules:int,vmp:float,target_voltage:float=800.0,*,voc:float=0.0,imp:float=0.0,isc:float=0.0,num_mppt:int=0,inputs_per_mppt:int=0,max_dc_v:float=0.0,mppt_min_v:float=0.0,mppt_max_v:float=0.0,max_current_mppt:float=0.0,max_isc_mppt:float=0.0)->StringDesign:
    ...
    warnings=_allocate_mppts(strings,int(num_mppt or 0),int(inputs_per_mppt or 0));checks=[]
    ...
    if max_current_mppt and imp and num_mppt:
        ...
        for m in range(1,num_mppt+1):
            count=sum(1 for s in strings if s.mppt==m)
            if not count:continue
            current=count*imp;details.append(f'MPPT{m} {current:.2f}A');fail|=current>max_current_mppt
    ...
    if max_isc_mppt and isc and num_mppt:
        ...
        for m in range(1,num_mppt+1):
            count=sum(1 for s in strings if s.mppt==m)
            if not count:continue
            current=count*isc;details.append(f'MPPT{m} {current:.2f}A');fail|=current>max_isc_mppt
